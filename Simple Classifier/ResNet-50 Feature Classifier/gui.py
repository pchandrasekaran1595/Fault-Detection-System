...
_, batch_size, lr, wd = Models.build_model(embed=u.embed_layer_size)
...
class VideoFrame(tk.Frame):
    ...
    def update(self):
        ...
        if not self.isResult:
            ...
            if ret:
                frame = __help__(frame=frame, model=self.model, pt1=(self.data[0], self.data[1]), pt2=(self.data[2], self.data[3]), show_prob=False, fea_extractor=Models.fea_extractor)
                self.image = ImageTk.PhotoImage(Image.fromarray(frame))
                self.canvas.create_image(0, 0, anchor="nw", image=self.image)
                self.id = self.after(self.delay, self.update)
            else:
...
class ButtonFrame(tk.Frame):
    ...
    def do_add(self):
        ...
        trainer(part_name=self.part_name, model=self.model, epochs=u.epochs, lr=lr, wd=wd, batch_size=batch_size)
